      ...
      <div class="podium-spot podium-2nd" data-anim>
        <div class="podium-avatar-wrap">
          <?php if ($p2['profile_pic']): ?>
            <img src="../uploads_v4d/profiles/<?= sanitize($p2['profile_pic']) ?>" alt="<?= sanitize($p2['name']) ?>" class="podium-avatar" style="border-color: <?= sanitize($p2['border_color']) ?>">
          <?php else: ?>
      ...
      <div class="podium-spot podium-1st" data-anim>
        ...
        <div class="podium-avatar-wrap">
          <?php if ($p1['profile_pic']): ?>
            <img src="../uploads_v4d/profiles/<?= sanitize($p1['profile_pic']) ?>" alt="<?= sanitize($p1['name']) ?>" class="podium-avatar podium-avatar-lg" style="border-color: <?= sanitize($p1['border_color']) ?>">
          <?php else: ?>
      ...
      <div class="podium-spot podium-3rd" data-anim>
        <div class="podium-avatar-wrap">
          <?php if ($p3['profile_pic']): ?>
            <img src="../uploads_v4d/profiles/<?= sanitize($p3['profile_pic']) ?>" alt="<?= sanitize($p3['name']) ?>" class="podium-avatar" style="border-color: <?= sanitize($p3['border_color']) ?>">
          <?php else: ?>
          ...
          <tr class="lb-row <?= $r <= 3 ? 'lb-row-top' : '' ?>" onclick="window.location='player.php?id=<?= $p['id'] ?>'" style="cursor:pointer;">
            ...
            <td class="lb-player-cell">
              <?php if ($p['profile_pic']): ?>
                <img src="../uploads_v4d/profiles/<?= sanitize($p['profile_pic']) ?>" alt="" class="lb-avatar">
              <?php else: ?>
    ...
    <div class="lb-mobile-list mt-4 lb-mobile">
      ...
      <a href="player.php?id=<?= $p['id'] ?>" class="lb-mobile-card <?= $r <= 3 ? 'lb-mobile-card-top' : '' ?>">
        ...
        <div class="lb-mc-left">
          <span class="rank-badge rank-<?= $r <= 3 ? $r : 'other' ?>">#<?= $r ?></span>
          <?php if ($p['profile_pic']): ?>
            <img src="../uploads_v4d/profiles/<?= sanitize($p['profile_pic']) ?>" alt="" class="lb-mc-avatar" style="border-color:<?= sanitize($p['border_color']) ?>">
          <?php else: ?>
