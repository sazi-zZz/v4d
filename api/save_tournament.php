...
try {
            ...
            if ($wins > 0 || $games > 0) {
                $upsert = $pdo->prepare(
                    "INSERT INTO tournament_stats (tournament_id, player_id, wins, games) VALUES (?,?,?,?)
                     ON DUPLICATE KEY UPDATE wins=?, games=?"
                );
                $upsert->execute([$id, $player_id, $wins, $games, $wins, $games]);
            } else {
    ...
    $_SESSION['flash'] = ['type'=>'error','msg'=>"Database error: could not save. This sometimes happens if the content or photos are too large."];
    redirect('../admin/tournaments.php?action=' . ($id ? 'edit&id='.$id : 'add'));
}
