...
if ($action === 'edit' && $edit_id) {
    ...
    $s->execute([$edit_id]);
    $edit_player = $s->fetch();
    if (!$edit_player) { redirect('/v4d/admin/players.php'); }
}
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="admin-layout">
  ...
  <main class="admin-main">
    <div class="admin-topbar">
      ...
      <?php else: ?>
        <a href="/v4d/admin/players.php" class="btn btn-outline btn-sm">← Back</a>
      <?php endif; ?>
    </div>
    ...
    <div class="admin-table-wrap glass-card">
      <?php if (empty($players)): ?>
        <div class="empty-state"><div class="empty-icon">👥</div><p>No players yet. <a href="?action=add" class="text-primary">Add one!</a></p></div>
      <?php else: ?>
          ...
          <tr>
            <td class="admin-player-cell">
              <?php if ($p['profile_pic']): ?>
                <img src="/v4d/uploads/profiles/<?= sanitize($p['profile_pic']) ?>" class="admin-avatar">
              <?php else: ?>
                <span class="admin-avatar-ph">🎮</span>
              <?php endif; ?>
              <span class="<?= font_class($p['font_style']) ?>"><?= sanitize($p['name']) ?></span>
            </td>
            <td><small><?= ucfirst($p['font_style']) ?></small></td>
            <td class="text-primary"><?= $p['total_wins'] ?></td>
            <td class="text-muted"><?= $p['total_games'] ?></td>
            <td><?= win_rate($p['total_wins'], $p['total_games']) ?></td>
            <td class="admin-actions-cell">
              <a href="?action=edit&id=<?= $p['id'] ?>" class="btn btn-outline btn-sm">Edit</a>
              <button class="btn btn-danger btn-sm" onclick="confirmDelete(<?= $p['id'] ?>, '<?= sanitize($p['name']) ?>')">Delete</button>
            </td>
          </tr>
          <?php endforeach; ?>
        </tbody>
    ...
    <?php else: ?>
    <!-- Add / Edit Form -->
    <form class="admin-form" method="POST" action="/v4d/api/save_player.php" enctype="multipart/form-data">
      <?php if ($edit_player): ?>
      ...
      <div class="form-group">
        ...
        <textarea id="bio" name="bio" class="form-control" rows="4" placeholder="Player bio, gaming history, achievements..."><?= sanitize($edit_player['bio'] ?? '') ?></textarea>
      </div>
      ...
      <div class="form-row">
        <div class="form-group">
          ...
          <?php if (!empty($edit_player['profile_pic'])): ?>
            <img src="/v4d/uploads/profiles/<?= sanitize($edit_player['profile_pic']) ?>" class="upload-preview-img" id="profile-preview" alt="Current profile pic">
          <?php else: ?>
        ...
        <div class="form-group">
          ...
          <?php if (!empty($edit_player['cover_image'])): ?>
            <img src="/v4d/uploads/covers/<?= sanitize($edit_player['cover_image']) ?>" class="upload-preview-cover" id="cover-preview" alt="Current cover">
          <?php else: ?>
      ...
      <div class="form-group">
        ...
        <div class="player-card preview-card"
             id="card-preview"
          ...
          <div class="player-card-body">
            ...
            <div class="player-card-name <?= font_class($edit_player['font_style'] ?? 'modern') ?>"
                 id="preview-name"
      ...
      <div class="admin-form-actions">
        ...
        <a href="/v4d/admin/players.php" class="btn btn-outline">Cancel</a>
      </div>
    </form>
    <?php endif; ?>
  </main>
...
<div class="modal-overlay" id="delete-modal" style="display:none;">
  <div class="modal-box">
    ...
    <div class="modal-actions">
      <form method="POST" action="/v4d/api/delete_player.php">
...
<script src="/v4d/js/admin.js"></script>
<script>
